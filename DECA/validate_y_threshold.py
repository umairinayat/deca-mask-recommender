...
class YThresholdValidator:
    ...
    def visualize(self, frame, verts, trans_verts, crop_info):
        ...
        # Calculate Y threshold in FLAME coordinates
        threshold_y = min_y + (max_y - min_y) * (self.percentile / 100.0)
        
        # Convert to image Y
        threshold_img_y = self.flame_y_to_image_y(threshold_y, trans_verts, crop_info)
        
        # Filter vertices below threshold (lower Y in FLAME = alar level)
        below_threshold = []
        above_threshold = []
        
        for idx in self.nose_vertices:
            v_y = verts[idx, 1]
            if v_y <= threshold_y:  # Below threshold (alar level)
        ...
        print(f"{'='*60}")
        print(f"Threshold: {self.percentile:.1f}% = FLAME Y {threshold_y:.4f}")
