...
class YThresholdSelector:
    def __init__(self):
        ...
        self.captured_frame = None  # Static captured frame for visualization
    ...
    def find_vertex_below_line(self, clicked_y):
        """
        Find the vertex with LOWEST image Y that is still BELOW the clicked line.
        Below in image means Y > clicked_y (lower down in image, toward chin).
        """
        ...
        # Get the one with lowest Y (closest to the line but still below it)
        selected = min(vertices_below, key=lambda v: v[2])
    ...
    def run(self):
        ...
        while True:
            ...
            # Show visualization on captured static frame (not live camera)
            if self.current_verts is not None and self.captured_frame is not None:
                vis = self.visualize(self.captured_frame)
            ...
            if key == ord(' '):
                ...
                verts, trans_verts, crop_info = self.process_frame(self.captured_frame)
                if verts is not None:
                    ...
                    self.selected_vertex = None
                    self.percentile = None
                    print(f"[OK] Captured frame and computed {len(self.vertex_image_coords)} vertex positions")
